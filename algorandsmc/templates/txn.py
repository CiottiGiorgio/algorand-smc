...
def smc_txn_refund(msig: str, sender: str, min_refund_block: int, max_refund_block: int) -> PaymentTxn:
    """
    ...
    if sugg_params.last < min_refund_block:
        raise ValueError("Suggested parameters for refund transaction are not compatible with min_refund_block")

    if sugg_params.first > max_refund_block:
        raise ValueError("Suggested parameters for refund transaction are not compatible with max_refund_block")
